...
const handleMobileFocus = (): void => {
	isDesktopSearch = false;
	requestPagefind();
};
...
// --- Reactive Statements ---
$: if (initialized) {
	search(isDesktopSearch ? keywordDesktop : keywordMobile, isDesktopSearch);
}
</script>
